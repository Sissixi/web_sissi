...
from PageLocators.ModuleBLoc.Check_booking_b_loc.booking_requirements_audit_details_pageloc import \
    Booking_requirements_AuditLoc as loc
...
class Review_detailsPage(BasePage):
    ...
    def status_is_check(self):
        return self.check_ele_visiable(loc.auide_state, "预约需求审核页面预约订单列表_预约订单状态")
    def execute_state_handle_js(self):
        self.js_scrollIntoView(loc.execute_state, "预约需求审核页面预约订单列表_滚动条到底部")
    def execute_state_is_check(self):
        return self.check_ele_visiable(loc.execute_state, "预约需求审核页面预约订单列表_预约订单执行状态")